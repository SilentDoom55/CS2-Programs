...
// Notes:
// Coronavirus (*) can be treated as a wall(#).
import java.io.*;
import java.util.*;
...
public class Pathogen
{
	...
	private static final char CORONA		 = '*';  // treated as a wall
	...
	public static HashSet<String> findPaths(char [][] maze)
	{
		...
		// Let's goooooooo!!
		findPaths(maze, visited, startRow, startCol, height, width, builder);
		return paths;
	}

	private static boolean findPaths(char [][] maze, char [][] visited,
	                                 int currentRow, int currentCol,
	                                 int height, int width, StringBuilder builder)
	{
		...
		for (int i = 0; i < moves.length; i++)
		{
			...
			findPaths(maze, visited, newRow, newCol, height, width, builder);
			...
			// Removes the characters that are being backtracked
	...
	{
		// Checks to make sure the new column and row are inbounds
